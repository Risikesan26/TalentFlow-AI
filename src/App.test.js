...
import { 
  CheckCircle, User, BookOpen, Award, MessageCircle, Search, FileText, Play, Star,
  ChevronRight, Bot, Target, Calendar, Zap, Trophy, Timer, ArrowUp, Bell, X, BarChart3,
  Users, TrendingUp, Moon, Sun,ChevronDown, Settings, LogOut,
  Rocket,
} from 'lucide-react';
...
const securityQuizQuestions = [
  {
    ...
    correctAnswer: 1,
    explanation: "Strong passwords should be complex and unique, combining different character types to make them harder to crack."
  },
  {
    id: 2,
    question: "What should you do if you receive a suspicious email asking for your login credentials?",
    options: [
    ...
    correctAnswer: 2,
    explanation: "Phishing emails should never be trusted. Always delete suspicious emails and report them to your IT security team."
  },
  {
    id: 3,
    question: "When working remotely, which Wi-Fi network should you avoid?",
    options: [
      "Your home network with WPA3 encryption",
      "Public Wi-Fi at coffee shops or airports",
      "Your mobile hotspot",
      "Your neighbor's secured network with permission"
    ],
    correctAnswer: 1,
    explanation: "Public Wi-Fi networks are unsecured and can be easily intercepted by attackers. Always use secure, encrypted networks."
  },
  {
    id: 4,
    ...
    correctAnswer: 2,
    explanation: "2FA adds an additional layer of security by requiring a second form of verification beyond just your password."
  },
  {
    id: 5,
    question: "If you suspect your computer has been infected with malware, what should you do first?",
    options: [
      "Continue working and ignore it",
      "Try to remove it yourself with online tools",
      "Disconnect from the network and contact IT immediately",
      "Restart your computer to fix the problem"
    ],
    correctAnswer: 2,
    explanation: "Isolating the infected device prevents spread to other systems, and IT professionals should handle malware removal."
  }
];
  ...
  try {
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer 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`,
...
const QuizComponent = ({ task, onComplete }) => {
  ...
  // Timer effect
  React.useEffect(() => {
    if (quizStarted && !showResults && !quizCompleted && timeLeft > 0) {
  ...
  }, [quizStarted, showResults, timeLeft, quizCompleted]);
  ...
  const handleNextQuestion = () => {
    if (currentQuestion < securityQuizQuestions.length - 1) {
  ...
  const handleQuizSubmit = () => {
    let correct = 0;
    securityQuizQuestions.forEach((question, index) => {
      if (selectedAnswers[index] === question.correctAnswer) {
        correct++;
      }
    });
    
    const score = Math.round((correct / securityQuizQuestions.length) * 100);
    setQuizScore(score);
    setShowResults(true);
    ...
    return (
      <div style={{ marginTop: '12px', textAlign: 'center' }}>
        <div style={{
          ...
          <h4 style={{ color: theme.text, marginBottom: '8px' }}>Security Awareness Quiz</h4>
          <p style={{ color: theme.textSecondary, fontSize: '14px', marginBottom: '12px' }}>
            Test your knowledge of security best practices
          </p>
          <ul style={{ 
            textAlign: 'left', 
            color: theme.textSecondary, 
            ...
            <li>{securityQuizQuestions.length} questions</li>
  ...
  if (showResults) {
    const passed = quizScore >= task.minimumScore;
    return (
      <div style={{ marginTop: '12px', textAlign: 'center' }}>
        <div style={{
          padding: '24px',
          backgroundColor: passed ? '#f0f9ff' : '#fef2f2',
          border: `2px solid ${passed ? '#0ea5e9' : '#ef4444'}`,
          borderRadius: '12px'
        }}>
          ...
          <p style={{ color: theme.textSecondary, fontSize: '14px' }}>
            {passed 
              ? `You passed! You got ${Math.round(quizScore * securityQuizQuestions.length / 100)} out of ${securityQuizQuestions.length} questions correct.`
              : `You need ${task.minimumScore}% to pass. Review the material and try again.`
            }
          </p>
  ...
  const currentQ = securityQuizQuestions[currentQuestion];
  const progress = ((currentQuestion + 1) / securityQuizQuestions.length) * 100;
    ...
    <div style={{ marginTop: '12px' }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: '14px', color: theme.text }}>
          Question {currentQuestion + 1} of {securityQuizQuestions.length}
        </span>
        <span style={{ 
          fontSize: '14px', 
          color: timeLeft < 60 ? '#ef4444' : theme.text,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        >
          {currentQuestion === securityQuizQuestions.length - 1 ? 'Submit Quiz' : 'Next Question'}
        </button>
      </div>
    </div>
  );
